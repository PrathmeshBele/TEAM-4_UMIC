...
def arrow_callback(str_msg):
	if str(str_msg)=='right'or'left':
		global direction
		direction=str(str_msg)
...
def image_callback(img_msg):
	...
	#([20, 90, 4], [90, 255, 100]), #green
      ...
      #  hough = cv2.HoughLinesP(lane, 1, np.pi / 180, 80, np.array([]), minLineLength = 5, maxLineGap = 50)
        ...
        if cx>190 and cy>390 and direction=='right':
        	stop_controls_r(rx,ry)
        if cx>190 and cy>390 and direction=='left':
